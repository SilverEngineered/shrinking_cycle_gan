...
def main():
    ...
    for epoch in range(0, args.epochs):
        ...
        if prec1 > max(all_acc):
            ...
            torch.save(model.state_dict(), "./model_dist/cifar10_{}{}_acc_{:.3f}_{}_{}_tem_{}_epoch{}.pt".format(args.arch, args.depth, prec1, args.optmzr, args.teacharch, args.temperature, epoch))
            print("\n>_ Deleting previous model file with accuracy {:.3f}% now...\n".format(max(all_acc)))
            if len(all_acc) > 1:
                os.remove("./model_dist/cifar10_{}{}_acc_{:.3f}_{}_{}_tem_{}_epoch{}.pt".format(args.arch, args.depth, max(all_acc), args.optmzr, args.teacharch, args.temperature, last_saved_epoch))
